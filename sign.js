...
async function createGitHubIssue(title, body) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showCustomAlert('회원가입에 실패했습니다. 다시 시도해 주세요.');
        }
    } catch (error) {
        console.error('Error during issue creation', error);
        showCustomAlert('회원가입 중 오류가 발생했습니다. 나중에 다시 시도해 주세요.');
    }
}
...
async function checkGitHubIssueForLogin(username, password) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showCustomAlert('로그인 오류가 발생했습니다. 다시 시도해 주세요.');
        }
    } catch (error) {
        console.error('Error during issue fetching', error);
        showCustomAlert('로그인 중 오류가 발생했습니다. 나중에 다시 시도해 주세요.');
    }
